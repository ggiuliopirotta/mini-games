...
def send_email(sender, body):
    ...
    try:
        ...
        server.sendmail(
            "gipopiro@gmail.com",
            "gipopiro@gmail.com",
            f"Subject: From {sender} via Streamlit\n\n{body}",
        )
        server.quit()
...
with col:
    sender_ = st.text_input(label="From", value="Anonymus")

body_ = st.text_area(label="Body", value="")

send_ = st.button(label="Send email", on_click=send_email, args=(sender_, body_))
