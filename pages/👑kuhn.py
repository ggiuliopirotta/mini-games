...
with cols[0]:
    train_ = st.button(label="Train bot", disabled=not node.is_root(), on_click=set_bot)

with cols[1]:
    deal_ = st.button(
        label="Deal",
        disabled=not node.is_root()
        or not bool(st.session_state.kuhn_game["bot_sigma"]),
        on_click=deal,
    )

with cols[2]:
    reset_ = st.button(
        label="Reset", disabled=not node.is_terminal(), on_click=reset_round
    )
...
for i in st.session_state.kuhn_game["rewards"]:
    game_rewards_df.loc[int(i) - 1] = [
        i,
        len(game_rewards[i]),
        np.mean(game_rewards[i]),
    ]
game_rewards_df.set_index("As", inplace=True)

st.write(game_rewards_df)

restart_ = st.button(label="Restart count", on_click=reset_count)
...
with st.expander("Settings"):
    ...
    bot_freeze_ = st.checkbox(
        ...
        help="If flagged, the bot won't be trained to exploit your specific strategy",
...
simulate_ = st.button(
    label="Simulate", disabled=not bool(user_position), on_click=run_simulation
)
...
if bool(rewards):
    ...
    st.markdown(
        ...
        Average: {:.3f}  
        """.format(
            np.sum(rewards), np.mean(rewards)
        )
    )
...
with cols[1]:
    st.link_button(label="CFR +", url="https://arxiv.org/pdf/1407.5042.pdf")
